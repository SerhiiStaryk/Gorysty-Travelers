...
@Component({
  ...
  templateUrl: './advices-detail.component.html',
  styleUrls: ['./advices-detail.component.scss']
})
export class AdvicesDetailComponent implements OnInit {
  ...
  private getOnePost(): void {
    ...
    this.postService.getOnePostFirebaseByType(id, 'advice').subscribe(
      data => {
        this.post = data.data();
    ...
    this.postService.getAllFirebasePostsID().subscribe(
      data => {
        ...
        if (nextIdx > this.arrIdx.length) {
          ...
          this.postService.getOnePostFirebaseByType(this.nextIdPost, 'advice').subscribe(
            a => {
              const post = a.data();
        ...
        if (previousIdx < 0) {
          ...
          this.postService.getOnePostFirebaseByType(this.previousIdPost, 'advice').subscribe(
            a => {
              const post = a.data();
